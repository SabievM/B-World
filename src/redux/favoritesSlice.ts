...
const favoritesSlice = createSlice({
    ...
    reducers: {
        ...
        toggleFavoriteGoogleBooks: (state, action: PayloadAction<BookResponseType>) => {
            const book = action.payload
            const isFavorite = state.favorites.some((fav) => fav.title === book.title)

            if (isFavorite) {
                state.favorites = state.favorites.filter((fav) => fav.title !== book.title)
            } else {
                state.favorites.push(book)
            }
